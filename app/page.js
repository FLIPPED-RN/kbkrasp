        ...
        <div className="relative flex justify-between xs:gap-1 xs:w-[370px] mx-auto sm:w-[550px] md:w-[750px] lg:w-[950px] xl:w-[1150px]">
          <div>
            <input
              ...
              className="border h-[60px] bg-transparent text-white border-[#ffffff] rounded-lg px-4 text-xl xs:w-[300px] xs:text-[15px] sm:w-[430px] md:w-[580px] md:text-[18px] lg:w-[750px] xl:w-[900px]"
            />
            {inputValue && (
              <div className="transform mt-2 xs:w-[300px] xs:text-[15px] sm:w-[430px] md:w-[580px] md:text-[18px] lg:w-[750px] xl:w-[900px]">
                <div className={` ${isGroupSelected ? '' : 'border-[#ffffff] rounded-lg overflow-hidden border bg-black text-white'}`}>
          ...
          <button className="h-[60px] bg-[#ffffff] rounded-lg w-[300px] text-2xl text-[#000000] hover:bg-[#000000] hover:text-[#ffffff] transition-all delay-100 hover:border-[#ffffff] border border-[#6e2fba] xs:w-[60px] xs:text-[15px] sm:w-[110px] md:w-[160px] md:text-[20px] lg:w-[190px] xl:w-[230px]" onClick={handleSearch}>Найти</button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className='mt-[50px] flex gap-9 mx-auto flex-col'>
            {Object.keys(groupData).map((day, index) => (
              <div key={index} className='grid border border-[#ffffff] rounded-2xl py-5 xs:w-[370px] mx-auto xs:px-2 sm:w-[550px] sm:px-3 md:w-[750px] lg:w-[950px] lg:px-5 xl:w-[1150px]'>
                {groupData[day].lessons.every(lesson => lesson.name === 'Пары нет') ? (
                  ...
                  <div>
                    <h1 className='text-3xl text-[#c6c93f]'>{day}</h1>
                    <p className='pt-2 pb-7 font-mono'>Дата: <span className='font-bold'>{formatDate(groupData[day].date).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric' })}</span></p>
                    <ul className='xs:text-[13px] sm:text-[15px] md:text-[18px] lg:text-[20px]'>
                      {groupData[day].lessons.map((lesson, lessonIndex) => (
                        <li key={lessonIndex} className='flex mt-5 border px-3 h-[70px] rounded-md border-[#ffffff] items-center flex-wrap xs:h-[100px] md:h-[120px]'>
                          <p className='font-mono xs:w-[80px]'>Пара <span className='font-bold text-[#c6c93f]'>{lesson.number}</span></p>
                          <p className='font-mono xs:w-[150px] md:w-[600px] lg:w-[250px]'>Время: <span className='font-bold text-[#c6c93f]'>{lesson.time}</span></p>
                          <p className='font-mono xs:w-[3`0px] md:w-[650px] lg:w-[350px] xl:w-[600px]'>Название: <span className='font-bold text-[#c6c93f]'>{lesson.name}</span></p>
                          {lesson.name !== 'Пары нет' && (
                            <>
                              <p className='font-mono xs:w-[300px] lg:w-[400px]'>Преподаватель: {lesson.teacher}</p>
                              <p className='font-mono'>Кабинет: <span className='text-[#c6c93f] font-bold'>{lesson.room}</span></p>
                            </>
                          )}
                        </li>
                      ))}
          ...
          <h1 className='flex justify-center items-center mt-10 border border-[#ffffff] rounded-lg h-[40px] xs:w-[370px] xs:text-[13px] sm:w-[550px] md:w-[750px] lg:w-[950px] xl:w-[1150px] mx-auto'>Начните вводить группу и выберите из списка :)</h1>
        )}
      </main>
    </>
  );
